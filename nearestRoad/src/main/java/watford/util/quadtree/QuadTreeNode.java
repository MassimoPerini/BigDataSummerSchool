...
import nearestRoad.Car;
import nearestRoad.Road;
 ...
 * @author Christopher A. Watford
 */
public class QuadTreeNode extends AbstractSpatialSet implements Serializable {
	...
	public LinkedList getNearestRoad (Car c)
	{
		QuadTreeNode nodeCar=this.search(c.getPoint());
		LinkedList nearStreets=this.findNearestInQuad(nodeCar);
		boolean nextTime=true;
		
		while (nextTime){
			if (nearStreets.size()>0)
				nextTime=false;
			QuadTreeNode p=nodeCar.parent;
			if (p!=null){
				QuadTreeNode children []=p.getChildren();
				for (int i=0;i<children.length;i++)
				{
					QuadTreeNode child=children[i];
					if (!child.getBounds().equals(nodeCar.getBounds()))
					{
						nearStreets.addAll(findNearestInQuad(child));
					}
				}
				nodeCar=p;
				}
	...
	private LinkedList findNearestInQuad (QuadTreeNode nodeCar){
		
		LinkedList nearestRoads=new LinkedList();
		List s=nodeCar.getLeafMembers();
		if (s!=null){
		for (int i=0;i<s.size();i++)
		{
			if (s.get(i) instanceof Road)
			{
				nearestRoads.add(s.get(i));
			}
		}
		}
	//	nodeCar.setVisited(true);
		
		QuadTreeNode [] subNodes=nodeCar.getChildren();
		if (subNodes!=null){
		for (int i=0;i<subNodes.length;i++)
		{
			nearestRoads.addAll(findNearestInQuad(children[i]));
		}
		}
		return nearestRoads;
	...
	public QuadTreeNode search(Point obj)
	{

		System.out.println(this.geometryContains(obj));
		if(this.children == null) {
			if (this.geometryContains(obj))
				return this;
		}
		
		synchronized(this.children) {
			QuadTreeNode contains = null;
			int sz = this.children.length;
			for(int ii = 0; ii < sz; ii++) {
				boolean test=false;
				test |= this.children[ii].geometryContains(obj);
				if(test) {
					contains=this.children[ii].search(obj);
				}
			}
			return contains;
		}
